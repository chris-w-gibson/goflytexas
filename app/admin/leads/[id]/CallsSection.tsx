...
/** Phone calls (AI-answered) linked to this lead, with transcripts inline. */
      ...
      {calls.map((c) => (
        <details key={c.id} className="border border-slate-200 rounded-lg" open={calls.length === 1}>
          <summary className="p-3 cursor-pointer select-none text-sm flex flex-wrap gap-3 items-center">
            ...
            {c.recordingUrl ? (
              <a href={c.recordingUrl} target="_blank" rel="noreferrer" className="text-xs text-sky-700 hover:underline">
                Recording ↗
              </a>
            ) : null}
