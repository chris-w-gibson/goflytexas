...
export default async function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: 'Archived', value: counts.archived, color: 'bg-slate-100 text-slate-700' },
  ];
    ...
    <div className="space-y-8">
      <section>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          ...
          {/* Jim 2026-09-24: "a name search box would be helpful on the landing page" */}
          <form method="get" action="/admin/leads" className="flex gap-2" role="search">
            <input
              type="search"
              name="q"
              placeholder="Find a lead by name, email or phone"
              aria-label="Search leads"
              className="w-72 max-w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
            />
            <button
              type="submit"
